...
class PriorityQueue:
    ...
    def append(self, item):
        ...
        # print(f"append: {self.f(item)} and {item}")
...
class Node:
    ...
    def child_node(self, problem, action):
        ...
        # print(f"Generating Child Node: {problem.result(self.state,action)}")
        next_state = problem.result(self.state, action)
        # print(f"{next_state} and the path cost is {problem.path_cost(self.path_cost, self.state, action, next_state)}")
...
def best_first_graph_search(problem, f):
    ...
    print("Searching...")
    node = Node(problem.initial)
    # print(f"bfgs, initial: {problem.initial}")
    
    if problem.goal_test(node.state):
        return node
    frontier = PriorityQueue(f=f)
    # print(f"bfgs, after test: {node}")
    frontier.append(node)
    # print(f"bfgs, after after append: {node}")
    explored = set() # set of states
    
    while frontier:
        node = frontier.pop()
        # print(f"The frontier node is {node}")
        if problem.goal_test(node.state):
            return node
        explored.add(node.state)
        # print(f"{node} with heuristic {problem.h(node)}")
        # print(f"There are {len(node.expand(problem))} child nodes")
